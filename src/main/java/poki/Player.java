...
public class Player {
    ...
    private void calculateHandStatistics(List<List<Double>> weights, int nrOpponents, boolean calculatePotential,
                                            boolean effectiveOdds, List<Card> unknownCards) throws HandRankingException{
        ...
        List<List<Integer>> combinations = Combinations.combine(unknownCards.size(), 2);
        // BoardCard Combinations
        List<List<Integer>> boardCombinations
                        = Combinations.combine(unknownCards.size()-2, effectiveOdds ? 2 : 1);
